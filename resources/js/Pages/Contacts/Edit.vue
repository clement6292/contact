<template>
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Modifier un contact</h1>
        ...
        <!-- Message d'erreur si contact non trouvé -->
        <div v-if="!contact || !contact.id" class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6 rounded-lg">
            Contact non trouvé. Veuillez retourner à la liste des contacts.
        </div>
...
const { contact, countries, errors } = defineProps({
    contact: {
        type: Object,
        default: () => ({}),
    },
...
const form = reactive({
    ...
    country_fly: contact.country_fly || '',
});
...
function updateSelectedCountry() {
    selectedCountry.value = countries.find(c => c.name === form.country) || null;
}

// Initialiser selectedCountry avec le pays du contact
if (contact && contact.country) {
    selectedCountry.value = countries.find(c => c.name === contact.country) || null;
}

// Soumettre le formulaire
function submit() {
    if (!contact || !contact.id) return; // Empêche la soumission si contact est invalide
